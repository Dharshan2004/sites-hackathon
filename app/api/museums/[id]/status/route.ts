...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (row.status === 'failed') return json({ error: 'The AI could not finish this museum. Try another photograph.' }, { status: 502 });
  if (!bindings.OPENAI_API_KEY || !row.render_response_id || !row.curation_response_id) return json({ error: 'This museum job cannot continue.' }, { status: 500 });
  ...
  try {
    ...
    if (render.status === 'failed' || render.status === 'cancelled') {
      ...
      return json({ error: 'The AI could not finish this museum. Try another photograph.' }, { status: 502 });
    }
    if (render.status !== 'completed') {
      const elapsed = Math.floor((Date.now() - row.created_at) / 1000);
      return json({ id, status: 'processing', message: elapsed > 55 ? 'Lighting the final gallery. Detailed renders can take about two minutes.' : 'Constructing your architectural world.' }, { status: 202 });
    }
    ...
    if (curation.status === 'completed') {
      ...
      return json({ id, status: 'processing', message: 'The curator is writing your exhibit labels.' }, { status: 202 });
    }
    ...
    return json({ error: 'The museum status could not be checked. Please try again.' }, { status: 502 });
  }
}
